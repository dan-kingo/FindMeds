...
const Medicines = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 rounded-3xl border border-fuchsia-200/70 bg-gradient-to-r from-fuchsia-50 to-pink-100 p-6 shadow-soft">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">
            Medicine Master List
          </h1>
          <p className="text-slate-600">
            Curate medicine records, types, and prescription requirements.
          </p>
        </div>
        <button
      ...
      <div className="card">
        <div className="card-body">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-fuchsia-500" />
            <input
              type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredMedicines.map((medicine) => (
          <div
            key={medicine._id}
            className="card border-t-4 border-fuchsia-300"
          >
            <div className="card-body">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex space-x-2 ml-4">
                  <button
                    ...
                    className="p-2 text-slate-400 hover:text-primary-600 rounded-lg hover:bg-fuchsia-50"
                  >
                    <Edit className="h-4 w-4" />
                  </button>
                  <button
                    onClick={() => handleDeleteMedicine(medicine._id)}
                    className="p-2 text-slate-400 hover:text-error-600 rounded-lg hover:bg-rose-50"
                  >
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">Prescription:</span>
                  <span
                    className={`badge ${
...
const MedicineModal: React.FC<MedicineModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const data = {
      ...formData,
      relatedNames: formData.relatedNames
        .split(",")
        .map((name) => name.trim())
        .filter(Boolean),
    };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white/95 backdrop-blur-sm shadow-xl rounded-2xl border border-fuchsia-100">
