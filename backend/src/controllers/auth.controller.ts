...
import { generateOtp, resendOtpToUser } from '../utils/otp.js';
...
export const requestOtp = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user) {
      user = await User.create({ phone, name, email, location, isVerified: true });
    } else {
    ...
    res.status(200).json({ message: 'Registration initiated (OTP disabled)', user });
  } catch (err: any) {
    console.error(err);
    res.status(500).json({ message: err.message || 'Failed to initiate registration' });
  }
};
...
export const forgotPasswordRequestOtp = async (req: Request, res: Response) => {
  ...
  await client.messages.create({
    body: `Your FindMeds password reset OTP is: ${otp}`,
    from: fromPhone,
    to: phone,
  });
