...
import { uploadMedicineImage } from '../services/cloudinary.service.js';
...
export const searchMedicines = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const medicines = await Medicine.find(medicineFilter)
      .select('_id name type strength price quantity unit description requiresPrescription imageUrl')
      .populate({
        path: 'pharmacy',
...
export const addMedicine = async (req: Request, res: Response) => {
  try {
    ...
    if (req.file) {
      try {
        imageUrl = await uploadMedicineImage(req.file);
      } catch (error) {
        console.error('Medicine image upload failed:', error);
         res.status(500).json({ error: 'Failed to upload medicine image' });
         return
      }
    }
...
export const updateMedicine = async (req: Request, res: Response) => {
  try {
    ...
    if (req.file) {
      try {
        updates.imageUrl = await uploadMedicineImage(req.file);
      } catch (error) {
        console.error('Medicine image upload failed:', error);
         res.status(500).json({ error: 'Failed to upload medicine image' });
         return
